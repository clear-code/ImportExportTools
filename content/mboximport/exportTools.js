...
function exportSelectedMsgs(type) {
	...
	*/
	...
	if (isOffLineImap) {
		var tempArray = [];
		var mms = messenger.messageServiceFromURI(eml)
	...
	if (type == 1) 
		...
		if (type == 5)	
			createIndex(type, file, hdrArray, msgFolder, true);
		else
			createIndexCSV(type, file, hdrArray, msgFolder, needBody);	
	}
	else
...
function IETrunExport(type,subfile,hdrArray,file2,msgFolder) {
	...
	switch (type) {
		...
		case 3:  // just HTML index
			createIndex(type, file2, hdrArray, msgFolder, true);
			break;
...
function createIndex(type, file2, hdrArray, msgFolder,justIndex) {
	...
	for (i=0;i<hdrArray.length;i++) {
		...
		if (! justIndex) {
			...
			var url = encodeURIComponent(subdirname)+'/'+encodeURIComponent(urlname)+ ext;
...
function saveMsgAsEML(msguri,file,append,uriArray,hdrArray,fileArray,imapFolder,clipboard,file2,msgFolder) {
    var myEMLlistner = {
		...
		if (IETexported < IETtotal) {
			...
			if (myEMLlistner.file2)
				createIndex(0, myEMLlistner.file2, hdrArray, myEMLlistner.msgFolder, false);
			IETexported = 0
			IETtotal = 0;
			IETskipped = 0;
				...
				if (saveAttachments) {
					...
					try {
						...
						for (var i=0;i<imgs.length;i++) {
							...
							var sep = isWin ? "\\" : "/";
							data = data.replace(aUrl, embImgContainer.leafName+sep+i+".jpg");
						}		
					}
					catch(e) {
